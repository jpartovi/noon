...
def find_audio_files(directory: Path) -> List[Path]:
    ...
    if not audio_files:
        ...
        print(
            f"   Common formats: mp3, mp4, mpeg, mpga, m4a, wav, webm, flac, ogg, opus, aac, mp2, 3gp"
        )
...
def test_rest_endpoint(audio_file: Path) -> str:
    ...
    try:
        ...
        with open(audio_file, "rb") as f:
            ...
            print(
                f"   Uploading {audio_file.name} ({audio_file.stat().st_size / 1024:.1f} KB)..."
            )
...
async def test_websocket_endpoint(audio_file: Path) -> str:
    ...
    try:
        async with websockets.connect(ws_url) as websocket:
            ...
            command = {"action": "transcribe", "filename": audio_file.name}

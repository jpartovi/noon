"""
Timing logger utility for backend performance debugging.
Writes timing information to a file instead of console.
Only logs when ENABLE_TIMING_LOGGER is set to True in config.
"""
...
_log_file_path = Path(__file__).parent.parent.parent / "backend_timing.log"


@lru_cache(maxsize=1)
def _is_enabled() -> bool:
    """Check if timing logging is enabled via config."""
    try:
        from core.config import get_settings
        return get_settings().enable_timing_logger
    except Exception:
        # If config can't be loaded, default to False
        return False
        ...
        with _lock:
            if not self.log_file.exists():
                with open(self.log_file, "w") as f:
                    f.write(f"=== Backend Timing Log - Started {datetime.now().isoformat()} ===\n\n")
